...
public class RobotKeyboard {
    ...
    private static final int NUMBER_END = 57;

    private static RobotKeyboard robotKeyboard = null;
    private Robot robot;
    ...
    public static RobotKeyboard getInstance() throws AWTException {
        if (robotKeyboard == null) {
            synchronized (RobotKeyboard.class) {
                if (robotKeyboard == null) {
                    robotKeyboard = new RobotKeyboard();
                }
            }
        }
    ...
    private void type(char character) {
        if (character >= NUMBER_START && character <= NUMBER_END){
            number(character);
        }else if (character >= UPPER_START && character <= UPPER_END){
            upper(character);
        }else if (character >= LOWER_START && character <= LOWER_END){
            lower(character);
        }else {
            other(character);
        }
    ...
    private void lower(char character){
        switch (character) {
    ...
    private void upper(char character){
        switch (character) {
    ...
    private void number(char character){
        switch (character) {
    ...
    private void other(char character){
        switch (character) {
